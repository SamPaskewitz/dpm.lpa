...
### standardize data
While not strictly necessary, the code is designed to deal with standardized (i.e. z-scored data).
```{r standardize_data}
...
The third column ("conclusion") reports the conclusion of the Bayes factor, which is that the profile means differ, are equal, or the evidence is not decisive. This is based on the conventional criterion of log10(BF) > 0.5 being decisive ** INSERT REF **.
...
NOTE TO SELF: fix the display of post-hoc results.
```{r bayes_factors}
result$bayes_factors
```
```{python}
r.result['bayes_factors'].loc[0, 'post_hoc_result']
```
